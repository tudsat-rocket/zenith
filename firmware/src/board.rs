...
pub struct Board {
    ...
    pub lora2: LoRa<LoraTransceiver, embassy_time::Delay>,
    pub usb: embassy_stm32::usb::Driver<'static, USB_OTG_FS>,
...
static SPI1_SHARED: StaticCell<embassy_sync::mutex::Mutex<CriticalSectionRawMutex, Spi<Async>>> =
    StaticCell::new();
static SPI2_SHARED: StaticCell<embassy_sync::mutex::Mutex<CriticalSectionRawMutex, Spi<Async>>> =
    StaticCell::new();
static SPI3_SHARED: StaticCell<embassy_sync::mutex::Mutex<CriticalSectionRawMutex, Spi<Async>>> =
    StaticCell::new();
static SPI4_SHARED: StaticCell<embassy_sync::mutex::Mutex<CriticalSectionRawMutex, Spi<Async>>> =
    StaticCell::new();
...
pub async fn init() -> Board {
    ...
    let imu1 = LSM6::init(SpiDevice::new(spi1, spi1_cs_imu1))
        .await
        .unwrap();
    let imu2 = ICM42688P::init(SpiDevice::new(spi2, spi2_cs_imu2))
        .await
        .unwrap();
    let imu3 = ICM42670P::init(SpiDevice::new(spi2, spi2_cs_imu3))
        .await
        .unwrap();
    let highg = H3LIS331DL::init(SpiDevice::new(spi1, spi1_cs_highg))
        .await
        .unwrap();
    ...
    let baro2 = LPS22::init(SpiDevice::new(spi2, spi2_cs_baro2))
        .await
        .unwrap();
    let baro3 = BMP580::init(SpiDevice::new(spi2, spi2_cs_baro3))
        .await
        .unwrap();
