...
impl<SPI: SpiDevice<u8>> ICM42688P<SPI> {
    pub async fn init(spi: SPI) -> Result<Self, SPI::Error> {
        ...
        if whoami != 0x47 {
            error!(
                "Failed to initialize ICM-42688-P (0x{:02x} != 0x47)",
                whoami
            );
        } else {
    ...
    async fn configure(&mut self) -> Result<(), SPI::Error> {
        ...
        self.write_u8(ICM42688PRegister::PwrMgmt0, 0b0000_1111)
            .await?;
        Ok(())
    }
    ...
    async fn read_sensor_data(&mut self) -> Result<(), SPI::Error> {
        let mut payload = [
            (ICM42688PRegister::TempData1 as u8) | 0x80,
            0,
            0,
            0,
            0,
        ...
        self.gyro = Some(
            Vector3::new(gyro_x.saturating_neg() as f32, gyro_z as f32, gyro_y as f32) * 2000.0
                / (i16::MAX as f32),
        );
        self.accel = Some(
            Vector3::new(
                accel_x.saturating_neg() as f32,
                accel_z as f32,
                accel_y as f32,
            ) * 16.0
                * G_TO_MS2
