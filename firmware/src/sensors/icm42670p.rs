...
impl<SPI: SpiDevice<u8>> ICM42670P<SPI> {
    pub async fn init(spi: SPI) -> Result<Self, SPI::Error> {
        ...
        if whoami != 0x67 {
            error!(
                "Failed to initialize ICM-42670-P (0x{:02x} != 0x67)",
                whoami
            );
        } else {
    ...
    async fn configure(&mut self) -> Result<(), SPI::Error> {
        self.write_u8(ICM42670PRegister::GyroConfig0, 0b0000_0101)
            .await?;
        self.write_u8(ICM42670PRegister::AccelConfig0, 0b0000_0101)
            .await?;
        self.write_u8(ICM42670PRegister::PwrMgmt0, 0b0000_1111)
            .await?;
        Ok(())
    }
    ...
    async fn read_sensor_data(&mut self) -> Result<(), SPI::Error> {
        let mut payload = [
            (ICM42670PRegister::TempData1 as u8) | 0x80,
            0,
            0,
            0,
            0,
        ...
        // let temp = ((payload[1] as i16) << 8) + (payload[2] as i16);
        ...
        self.gyro = Some(
            Vector3::new(gyro_y as f32, gyro_z as f32, gyro_x as f32) * 2000.0 / (i16::MAX as f32),
        );
        self.accel = Some(
            Vector3::new(accel_y as f32, accel_z as f32, accel_x as f32) * 16.0 * G_TO_MS2
                / (i16::MAX as f32),
        );
