...
async fn run_lora_loopback_test(
    tx: &mut LoRa<fw::LoraTransceiver, Delay>,
    rx: &mut LoRa<fw::LoraTransceiver, Delay>,
) -> Option<PacketStatus> {
    ...
    let rx_pkt_params = rx
        ...
        .unwrap();
    rx.prepare_for_rx(RxMode::Continuous, &mdltn_params, &rx_pkt_params)
        .await
        .unwrap();
    ...
    // Finish RX
    let (len, status) = rx
        .complete_rx(&rx_pkt_params, &mut receiving_buffer)
        .await
        .unwrap();
...
async fn run_can_loopback_test(tx: &mut Can<'_>, rx: &mut Can<'_>) -> bool {
    ...
    let tx_future = async move {
        let frame =
            embassy_stm32::can::frame::Frame::new_standard(TEST_FRAME_ID, TEST_FRAME_CONTENT)
                .unwrap();
    ...
    let Ok((_, frame)) = with_timeout(Duration::from_secs(1), join(tx_future, rx_future)).await
    else {
        return false;
    };
...
macro_rules! baro_test {
    ($name:expr, $sensor:expr, $hertz:expr) => {{
        ...
        let var_pressure = pressures
            .iter()
            .map(|v| (mean_pressure - v).powi(2))
            .sum::<f32>()
            / len;
        let std_dev_pressure = var_pressure.sqrt();

        info!(
            "[SUCCESS] {} (pressure: {}, stddev: {}, temp: {})",
            $name, mean_pressure, std_dev_pressure, mean_temp
        );
    }};
}
...
async fn main(spawner: Spawner) {
    ...
    let (stack, runner) = embassy_net::new(
        board.ethernet,
        config,
        ETH_RESOURCES.init(StackResources::new()),
        board.seed,
    );
